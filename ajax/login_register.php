...
//require('/Applications/XAMPP/xamppfiles/htdocs/Licenta/inc/sendgrid1/sendgrid-php.php');
require 'send/vendor/autoload.php';
  

  

// Definirea funcției send_mail
...
function send_mail($uemail,$token,$type)
  ...
  { 
    ...
    $u_exist = select("SELECT * FROM `user_crew` WHERE `email` = ? OR `phonenum` = ? LIMIT 1",
    ...
    $query="INSERT INTO `user_crew`( `name`, `email`, `address`, `phonenum`, `pincode`, `dob`, 
      `profile`,`password`,`token`) VALUES (?,?,?,?,?,?,?,?,?)";
    $values=[$data['name'],$data['email'],$data['address'],$data['phonenum'],$data['pincode'],$data['dob'],$img,
      $enc_pass,$token];
