...
$tempDir = __DIR__ . '/Download';


if (!is_dir($tempDir) || !is_writable($tempDir)) {
    die("Directorul temporar '$tempDir' nu există sau nu este accesibil.");
}
  ...
  if(isset($_GET['gen_pdf']) && isset($_GET['id']))
  {
    ...
    $query = "SELECT bo.*, bd.*,uc.email FROM `booking_order` bo
      ...
      INNER JOIN `user_crew` uc ON bo.user_id = uc.id
    ...
    $mpdf = new \Mpdf\Mpdf([
      'tempDir' => $tempDir,
      'mode' => 'utf-8',
      'format' => 'A4-L' // Orientare pe orizontală
  ]);
